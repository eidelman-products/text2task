-- Text2Task Project Update client-detail mutation path safety
-- Migration: 202606160002_fix_client_detail_mutation_json_paths.sql
-- Created: 2026-06-16
--
-- Replaces apply_project_update_transaction with the current transactional
-- apply body, preserving the 202606160001 timeline metadata guard, and narrows
-- the client_detail_change JSON handling to one safe path extraction per
...
comment on function public.apply_project_update_transaction(
  ...
  'Atomically applies one claimed Project Update review and commits its item, mutation, timeline, and lifecycle writes. Patched 2026-06-16 to use safe client-detail JSONB path extraction.';
