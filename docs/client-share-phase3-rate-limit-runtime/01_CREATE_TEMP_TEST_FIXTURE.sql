...
--      Phase 3 rate-limit runtime-verification test project, which files
--      02 and 03 both require before they will run anything else.
--   3. Creates the smallest coherent test-only versions of the six
--      existing Text2Task tables the Client Share Link migrations
--      reference by foreign key or by trigger-body SELECT: projects,
--      tasks, clients, task_resources, project_updates,
--      project_timeline_events.
--   4. Creates one deterministic auth.users test identity (Owner A), used
--      throughout file 03 to create the one project_share_links row
--      needed to exercise the new RPC's `scope = 'share_link'` path.
--
-- Relationship to the Phase 2B mapping-read runtime package
-- (docs/client-share-phase2b-mapping-read-runtime/01_CREATE_TEMP_TEST_
-- FIXTURE.sql): this file is byte-for-byte identical except for its
-- sentinel table name, its project_kind value, and dropping the second
-- (Owner B) identity -- this package's own tests are entirely identity-
-- digest-based, not auth.uid()-ownership-based, so no cross-tenant
-- scenario is exercised here and a second owner identity is unnecessary.
--
...
-- reference, and creates only the minimum needed so that
-- 202608030003_client_share_owner_foundation.sql's own foreign keys can
-- resolve when the full cumulative migration bundle (file 02) is applied.
--
-- Deterministic identity namespace (documented here and in
-- 00_READ_ME_FIRST.md):
--   Owner A (auth.users.id): 11111111-1111-4111-8111-111111111111
-- This is the only FIXED UUID literal this fixture creates. The project
-- and project_share_links fixture ROWS this package's tests need are
-- created inside file 03's own transaction, referenced there by a stable
-- symbolic key rather than by a second layer of hard-coded literals.
...
do $$
begin
  if to_regclass('public.text2task_client_share_phase3_rate_limit_runtime_sentinel') is not null then
    raise exception using errcode = 'P0001', message =
      'REFUSING TO RUN. The Phase 3 rate-limit runtime test sentinel already exists in this project, which means file 01 has already been run here. Re-running it would not be a clean baseline. If you intend to start over, create a brand-new temporary Supabase project instead of reusing this one.';
  end if;
end;
$$;
...
-- Phase 3 rate-limit runtime-verification test project. Files 02 and 03
-- both refuse to run unless this row exists. Deliberately a DIFFERENT
-- table name from every other Client Share runtime package's own
-- sentinel, so no package can ever be confused with another.
-- =========================================================

create table public.text2task_client_share_phase3_rate_limit_runtime_sentinel (
  id boolean primary key default true,
  project_kind text not null default 'DISPOSABLE_PHASE_3_RATE_LIMIT_RUNTIME_TEST_PROJECT',
  fixture_version integer not null default 1,
  created_at timestamptz not null default now(),

  constraint text2task_client_share_phase3_rate_limit_runtime_sentinel_singleton_check
    check (id)
);

comment on table public.text2task_client_share_phase3_rate_limit_runtime_sentinel is
  'TEST-ONLY RUNTIME FIXTURE -- NOT A PRODUCTION MIGRATION. Proves this Supabase project is a disposable Phase 3 rate-limit runtime-verification test project, never the real Text2Task production project. Files 02 and 03 refuse to run unless this row exists.';

insert into public.text2task_client_share_phase3_rate_limit_runtime_sentinel default values;

-- =========================================================
-- 2. Minimal test-only Text2Task base schema
--
-- Only the columns the Client Share migrations (003-005, 050001,
-- 060001-3, 202608110001, 202608110002, 202608130001) actually reference
-- by foreign key, by trigger-body SELECT, or by RPC-body SELECT. Verified
...
-- tasks uses a bigint identity id, matching public.tasks.id in the real
-- schema.
-- =========================================================
...
comment on table public.tasks is
  'TEST-ONLY RUNTIME FIXTURE -- NOT A PRODUCTION MIGRATION. Minimal stand-in for public.tasks. id is bigint generated always as identity, matching the real production tasks.id type.';
...
comment on table public.project_updates is
  'TEST-ONLY RUNTIME FIXTURE -- NOT A PRODUCTION MIGRATION. Minimal stand-in for public.project_updates, containing only the columns enforce_share_message_conversion_integrity() reads. Not touched by any RPC exercised in this package.';
...
--    ownership under an authenticated caller identity, matching every
--    prior Client Share runtime package's fixture exactly.
-- =========================================================
...
-- =========================================================
-- 4. One deterministic auth.users test identity
--
...
insert into auth.users (
  ...
  'authenticated',
  'authenticated',
  'phase3-rate-limit-runtime-fixture-owner-a@example.invalid',
  'phase3-rate-limit-runtime-fixture-not-a-real-credential',
  now(),
  now(),
  now(),
