-- Text2Task Transactional Project Update Apply
-- Migration: 202606150008_transactional_project_update_apply.sql
-- Created: 2026-06-15
--
-- Phase 3A.2:
-- Atomically applies one claimed Project Update review, including accepted
-- mutations, rejected item state, timeline events, and final lifecycle state.
--
-- A unique timeline source-item index is intentionally deferred until
-- production duplicates can be audited without taking a write lock. This RPC
-- prevents duplicates by locking the update attempt and rejecting any existing
-- timeline event for an accepted source item before mutation.
    ...
    if v_item.type = 'new_subtask' then
      ...
      if v_updates = '{}'::jsonb
        ...
        or (
          ...
          and (v_updates - array['priority']) <> '{}'::jsonb
        )
        or (
      ...
      if v_mutation->>'kind' is distinct from 'client_detail'
        or jsonb_typeof(v_mutation->'projectUpdates') <> 'object'
        or jsonb_typeof(v_mutation->'clientUpdates') <> 'object'
        or jsonb_typeof(v_mutation->'taskUpdates') <> 'object' then
        raise exception using
          errcode = 'P0001',
      ...
      if (
          v_mutation->'projectUpdates' - array['client_name', 'contact_name']
        ) <> '{}'::jsonb
        or (
          v_mutation->'clientUpdates' - array[
            'name',
            'contact_name',
            'phone',
            'email',
        ...
        or (
          v_mutation->'taskUpdates' - array['client_name', 'contact_name']
        ) <> '{}'::jsonb then
        raise exception using
          errcode = 'P0001',
          message = 'INVALID_CLIENT_UPDATE_PAYLOAD';
      ...
      if v_mutation->'projectUpdates' <> '{}'::jsonb then
        v_updates := v_mutation->'projectUpdates';
      ...
      if v_project.client_id is not null
        and v_mutation->'clientUpdates' <> '{}'::jsonb then
        v_updates := v_mutation->'clientUpdates';
      ...
      if v_mutation->'taskUpdates' <> '{}'::jsonb then
        v_updates := v_mutation->'taskUpdates';
...
comment on function public.apply_project_update_transaction(
  ...
  'Atomically applies one claimed Project Update review and commits its item, mutation, timeline, and lifecycle writes.';
