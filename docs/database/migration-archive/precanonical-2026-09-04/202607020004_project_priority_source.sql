-- Text2Task Project Priority Provenance
-- Migration: 202607020004_project_priority_source.sql
-- Created: 2026-07-02
--
-- Adds durable project-level priority provenance so a DB-compatible stored
-- priority can be distinguished from a semantically real AI/user priority.

alter table public.projects
  add column if not exists priority_source text not null default 'unknown';

alter table public.projects
  alter column priority_source set default 'unknown';

comment on column public.projects.priority_source is
  'Project priority provenance: ai, user, storage_default, or unknown. storage_default means the stored priority is only a DB-compatible fallback.';
...
begin
  if not exists (
    ...
    where conname = 'projects_priority_source_check'
      and conrelid = 'public.projects'::regclass
  ) then
    alter table public.projects
      add constraint projects_priority_source_check
      check (priority_source in (
        'ai',
        'user',
        'storage_default',
        'unknown'
...
comment on function public.import_projects_transaction(uuid, uuid, text, jsonb) is
  'Server-only transactional import that verifies the normalized payload stored on the matching Project Import attempt and persists project priority provenance.';
