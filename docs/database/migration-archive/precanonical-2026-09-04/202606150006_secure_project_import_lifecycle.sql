-- Text2Task Secure Project Import Lifecycle
-- Migration: 202606150006_secure_project_import_lifecycle.sql
-- Created: 2026-06-15
--
-- Phase 3D.2E:
-- Makes Project Import attempts server-controlled and binds the transactional
-- import payload to trusted data stored when the server route claims an
-- attempt.

alter table public.project_import_attempts
  add column if not exists payload_json jsonb null,
  add column if not exists last_seen_at timestamptz not null default now();

create index if not exists project_import_attempts_started_last_seen_idx
  on public.project_import_attempts(last_seen_at)
...
revoke insert, update on table public.project_import_attempts
  from authenticated;

comment on column public.project_import_attempts.payload_json is
  'Trusted normalized import payload stored by the server route and verified by the transactional import RPC.';

comment on column public.project_import_attempts.last_seen_at is
  'Operational heartbeat for identifying stale started attempts. Stale attempts are never retried automatically.';
...
comment on function public.import_projects_transaction(uuid, uuid, text, jsonb) is
  'Server-only transactional import that verifies the normalized payload stored on the matching Project Import attempt.';
