...
--      Phase 1C runtime-verification test project, which files 02 and 03
...
--
-- Relationship to the Phase 1B runtime package
-- (docs/client-share-phase1b-runtime/01_CREATE_TEMP_TEST_FIXTURE.sql): this
-- file is byte-for-byte identical except for its sentinel table name and
-- project_kind value, deliberately renamed so the two packages can never be
-- confused with each other -- a project set up for Phase 1B is not silently
-- accepted as a valid Phase 1C baseline, and vice versa. Every other table,
-- column, RLS policy and grant below is unchanged from the Phase 1B fixture:
-- Phase 1C adds no new fixture-table dependency (it extends
-- project_share_links, an already-created Client Share table, not any of
-- the six base tables this fixture stands in for).
--
...
-- 202608030005_client_share_integrity_and_security.sql, and the fourteen
-- owner-authenticated RPCs delivered through Phase 1B and Phase 1C.
--
...
-- file 03's own transaction, exactly like the Phase 1B precedent, and
...
do $$
begin
  if to_regclass('public.text2task_client_share_phase1c_runtime_sentinel') is not null then
    raise exception using errcode = 'P0001', message =
      'REFUSING TO RUN. The Phase 1C runtime test sentinel already exists in this project, which means file 01 has already been run here. Re-running it would not be a clean baseline. If you intend to start over, create a brand-new temporary Supabase project instead of reusing this one.';
  end if;
end;
$$;
...
-- Phase 1C runtime-verification test project. Files 02 and 03 both refuse
-- to run unless this row exists. Deliberately a DIFFERENT table name from
-- the Phase 1B package's own sentinel
-- (text2task_client_share_phase1b_runtime_sentinel), so the two packages
-- can never be confused with each other.
-- =========================================================

create table public.text2task_client_share_phase1c_runtime_sentinel (
  id boolean primary key default true,
  project_kind text not null default 'DISPOSABLE_PHASE_1C_RUNTIME_TEST_PROJECT',
  fixture_version integer not null default 1,
  created_at timestamptz not null default now(),

  constraint text2task_client_share_phase1c_runtime_sentinel_singleton_check
    check (id)
);

comment on table public.text2task_client_share_phase1c_runtime_sentinel is
  'TEST-ONLY RUNTIME FIXTURE -- NOT A PRODUCTION MIGRATION. Proves this Supabase project is a disposable Phase 1C runtime-verification test project, never the real Text2Task production project. Files 02 and 03 refuse to run unless this row exists.';

insert into public.text2task_client_share_phase1c_runtime_sentinel default values;

-- =========================================================
-- 2. Minimal test-only Text2Task base schema
--
-- Only the columns the Client Share migrations (003-005, 050001,
-- 060001-3, 202608110001) actually reference by foreign key, by
-- trigger-body SELECT, or by RPC-body SELECT. Verified production
...
comment on table public.project_updates is
  'TEST-ONLY RUNTIME FIXTURE -- NOT A PRODUCTION MIGRATION. Minimal stand-in for public.project_updates, containing only the columns enforce_share_message_conversion_integrity() reads. Not touched by any Phase 1B/1C RPC in this package.';
...
insert into auth.users (
  ...
  (
    ...
    'authenticated',
    'authenticated',
    'phase1c-runtime-fixture-owner-a@example.invalid',
    'phase1c-runtime-fixture-not-a-real-credential',
    now(),
    now(),
    now(),
  ...
  (
    ...
    'authenticated',
    'authenticated',
    'phase1c-runtime-fixture-owner-b@example.invalid',
    'phase1c-runtime-fixture-not-a-real-credential',
    now(),
    now(),
    now(),
