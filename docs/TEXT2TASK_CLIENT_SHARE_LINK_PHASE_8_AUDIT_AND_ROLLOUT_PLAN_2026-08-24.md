...
```
ROLLOUT_STATUS: CORRECTIVE FIX DEPLOYED / PRODUCTION VERIFIED
```

**This rollout was PAUSED 2026-08-25 at the same-browser Re-enable regression, and is now COMPLETE for the corrective fix itself — the fix has been applied to Production, deployed, and the exact original regression has been re-run against Production and PASSED.** This document's own §22 smoke-test checklist was originally run and the Re-enable step **failed**: an already-authorized browser, holding the same unchanged URL, remained permanently denied after Disable → Re-enable, with no recovery path short of a brand-new browser/incognito session. Root cause (full detail in the implementation report linked below): `project_share_links.configuration_version` was, since Phase 1B, overloaded as **both** a presentation/owner-editor freshness signal **and** the sole security-grant staleness predicate `share_session_grants` was checked against at public read time. A mechanical, broader audit (not just this one bug) proved the same defect family also affects `clear_share_link_pin`, `set_share_link_expiry`, `clear_share_link_expiry`, and `save_share_configuration`'s settings sub-block — every one of them bumps `configuration_version` for legitimate presentation reasons, and every one of them was therefore silently, permanently stranding already-authorized browsers.

**Corrective fix implemented, disposable-runtime-verified, and now APPLIED TO PRODUCTION.** The migration `supabase/migrations/202608250001_client_share_access_epoch.sql` introduces two dedicated security-generation counters — `access_epoch` (bumped only by `rotate_share_link_secret`) and `pin_epoch` (bumped only by `set_share_link_pin`) — fully separating security-grant invalidation from `configuration_version`, which is left completely unchanged and continues to serve presentation freshness exactly as before. A companion expiry-staleness defect (grant expiry frozen at grant-creation time, unable to reflect a later-lengthened or cleared link expiry) was closed in the same change.

**Disposable-runtime disposition (2026-08-26, prior to Production application):** a disposable Supabase runtime-verification package (`docs/client-share-phase8-access-epoch-runtime/`) was built and executed against a real disposable Postgres instance. The final scripted run reported **138/139 PASS**; the sole failure was a harness-only privilege-assertion bug (not a migration/trigger/RPC defect), independently confirmed correct at the database level by a direct read-only PostgreSQL catalog query and since corrected in the harness. Zero implementation or migration defects were found. Full evidence: `docs/client-share-phase8-access-epoch-runtime/04_CAPTURE_RESULTS.md`.

**Production rollout (2026-08-26) — COMPLETE:**
- Commit `fa86c99` ("Fix Client Share access epoch recovery") pushed to `origin/main`; `main` and `origin/main` synchronized.
- Vercel Production deployment of commit `fa86c99`, branch `main`, environment Production, status **Ready**.
- Production migration `202608250001_client_share_access_epoch.sql` applied — **SUCCESS** ("Success. No rows returned"). Migration SHA-256 verified `dbc8af2f6581abb8c4dcbe74d8a94ddce46a26854491d24d579d9e6302a2be4e` before execution.
- Production preflight (before migration): `share_link_count=1`, `grant_count=2`, `live_grants_with_config_version_mismatch=1`; `access_epoch`/`pin_epoch`/`granted_access_epoch`/`granted_pin_epoch` all absent (as expected, pre-migration); `grant_integrity_function_present`/`rotate_secret_function_present`/`set_pin_function_present` all `true`.
- Production post-migration verification: `share_link_count=1`, `grant_count=2`, `links_access_epoch_not_1=0`, `links_pin_epoch_not_1=0`, `grants_access_epoch_not_1=0`, `grants_pin_epoch_not_1=0`, `epoch_mismatch_count=0`; `rotate_bumps_access_epoch`/`set_pin_bumps_pin_epoch`/`grant_checks_access_epoch`/`grant_checks_pin_epoch`/`old_expiry_rejection_removed` all `true`. `legacy_config_mismatch_count=1` — **expected**: this is the pre-existing stale grant the original bug produced; the new authorization code no longer reads `configuration_version` as the public authorization predicate, so this legacy mismatch is inert.
- **Production smoke test — the exact original regression, re-run live:** fresh Incognito → link loads (PASS) → owner Disable → same tab refresh → correctly denied (PASS, disable still blocks the already-authorized session) → owner Re-enable → **same** Incognito tab, **same** URL, no new browser, no new secret exchange → refresh → **loads successfully again** (PASS, same-browser access recovered). This is the exact scenario that originally failed in Production before the corrective fix.
- **FINAL VERDICT:** `PHASE 8 CORRECTIVE FIX: PRODUCTION VERIFIED` / `DB MIGRATION: PASS` / `DEPLOYMENT fa86c99: PASS` / `DISABLE BLOCKS EXISTING SESSION: PASS` / `RE-ENABLE RESTORES SAME SESSION: PASS` / `ORIGINAL PRODUCTION REGRESSION: FIXED`.

Full detail: `docs/TEXT2TASK_CLIENT_SHARE_LINK_ACCESS_EPOCH_IMPLEMENTATION_REPORT_2026-08-25.md` (§14, new) and `docs/client-share-phase8-access-epoch-runtime/04_CAPTURE_RESULTS.md`.

**This rollout plan (§1–27 below) is otherwise still accurate** for everything it covers (migration chain, risk matrix, env inventory, security audits of every OTHER capability, rollback strategy, etc.). §25's blocker-table row for this specific defect is now RESOLVED and Production-verified (see below). **This does NOT mean every other Client Share capability in this plan's own §22 smoke-test checklist has been manually re-verified in Production** — only the exact same-browser Disable→Re-enable scenario above was re-run; the remainder of §22's checklist (rotate, PIN set/change/clear, expiry set/clear/lengthen, settings changes, revoke, etc.) has not been reported as manually smoke-tested and its checkboxes remain as previously left.

**No code, migration, or fix in this update was applied to Production by this documentation-only edit.** This edit records a Production rollout and verification that has already occurred, reported directly by the user; it performs no action of its own.
...
- [x] Re-enable — verify it works again — **FAILED 2026-08-25 in the exact form below; fix implemented in `202608250001_client_share_access_epoch.sql`, RUNTIME-VERIFIED 2026-08-26 against a disposable, non-Production Postgres engine, and PRODUCTION-VERIFIED 2026-08-26 by re-running this exact sequence live. See the ROLLOUT STATUS UPDATE banner at the top of this document and §25's amended row for full detail.**
  - [x] Active link → open in a fresh browser → loads — **PASS (Production, 2026-08-26, fresh Incognito)**
  - [x] Disable → refresh the SAME browser/tab → correctly unavailable — **PASS (Production, 2026-08-26, "This shared project view is not available.")**
  - [x] Re-enable → refresh the SAME browser/tab, SAME URL, no re-copy, no new incognito, no raw secret → **must load again** — **PASS (Production, 2026-08-26, same Incognito tab, same URL, loaded successfully)**
...
|---|---|---|
| Same-browser Disable→Re-enable recovery (§22 smoke test) | **RESOLVED — PRODUCTION VERIFIED (2026-08-26)** | Found 2026-08-25 by actually running this document's own §22 smoke-test checklist: Disable → same browser correctly denied → Re-enable → same browser, same URL, still denied — no recovery without a brand-new browser. Root cause: `configuration_version` was overloaded as both presentation-freshness signal and the sole security-grant staleness predicate; every operation that bumps it for a legitimate presentation reason (disable, re-enable, clear PIN, set/clear expiry, `save_share_configuration` settings) was silently invalidating already-authorized browsers. Corrective fix: new migration `202608250001_client_share_access_epoch.sql` (dedicated `access_epoch`/`pin_epoch` security counters, `configuration_version` left untouched) — full detail in `docs/TEXT2TASK_CLIENT_SHARE_LINK_ACCESS_EPOCH_IMPLEMENTATION_REPORT_2026-08-25.md`. Locally verified (2904/2904 Client Share suite passing, TypeScript clean, production build clean), disposable-runtime-verified 2026-08-26 (final scripted run 138/139 PASS, sole failure independently confirmed a harness-only bug via a direct read-only catalog query, zero implementation/migration defects found — `docs/client-share-phase8-access-epoch-runtime/04_CAPTURE_RESULTS.md`), and **applied to Production 2026-08-26** (commit `fa86c99`, migration SHA-256 `dbc8af2f6581abb8c4dcbe74d8a94ddce46a26854491d24d579d9e6302a2be4e` verified, migration execution SUCCESS, post-migration verification clean — `epoch_mismatch_count=0`; the one remaining `legacy_config_mismatch_count=1` is the expected inert stale grant the original bug produced). The exact original regression was re-run live in Production and **PASSED**: fresh Incognito load → Disable denies the same session → Re-enable restores the **same** session with no new browser/URL/secret exchange. §21 (flag enablement) may now proceed per its own remaining gates below — this row no longer blocks it. |
