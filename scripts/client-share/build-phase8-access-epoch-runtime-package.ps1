...
.DESCRIPTION
  ...
  pre-canonical Client Share migrations) verbatim, in order, bringing
  ...
  File 02C bundles the ONE historical migration under test,
  202608250001_client_share_access_epoch.sql, verbatim, with a SHA-256
  header so its identity can be checked against the archived source file
  before it is ever pasted into a disposable project's SQL Editor.
  ...
  Same guarantees as every prior generator in this family:
    ...
    - reads the pre-canonical source migrations from the canonical
      historical archive adopted after migration governance recovery
...
$historicalMigrationArchiveDir = Join-Path $repoRoot 'docs\database\migration-archive\precanonical-2026-09-04'
...
#    access-epoch migration under test. Read-only against the
#    pre-canonical migration archive -- only Get-Content, never a write.
#    Fails closed (throws) if any is missing.
# ---------------------------------------------------------------------
...
foreach ($name in $prerequisiteSourceFiles) {
  $path = Join-Path $historicalMigrationArchiveDir $name
  $content = Get-NormalizedContent -Path $path
  $prereqContents[$name] = $content
  $prereqHashes[$name] = Get-Sha256Hex -Text $content
  $prereqRelativePaths[$name] = "docs/database/migration-archive/precanonical-2026-09-04/$name"
  Write-Host "Read prerequisite migration: $name  sha256=$($prereqHashes[$name])"
}

$accessEpochPath = Join-Path $historicalMigrationArchiveDir $accessEpochMigrationFile
...
-- under docs/database/migration-archive/precanonical-2026-09-04/ and
...
-- docs/database/migration-archive/precanonical-2026-09-04/$accessEpochMigrationFile
...
-- the SHA-256 below matches docs/database/migration-archive/precanonical-2026-09-04/$accessEpochMigrationFile
-- as it exists in the repository right now (e.g.
-- ``Get-FileHash -Algorithm SHA256 docs\database\migration-archive\precanonical-2026-09-04\$accessEpochMigrationFile``,
...
$sourceComment = "-- Source: docs/database/migration-archive/precanonical-2026-09-04/$accessEpochMigrationFile`n-- SHA-256 (normalized LF UTF-8): $accessEpochHash`n"
...
$manifestLines.Add('| Migration | SHA-256 |')
$manifestLines.Add('|---|---|')
$manifestLines.Add("| ``docs/database/migration-archive/precanonical-2026-09-04/$accessEpochMigrationFile`` | ``$accessEpochHash`` |")
$manifestLines.Add('')
$manifestLines.Add('**This is the value to independently verify against the archived repository source file**')
...
$manifestLines.Add('')
$manifestLines.Add('- The generator is read-only against `docs/database/migration-archive/precanonical-2026-09-04/**`.')
...
Write-Host "docs/database/migration-archive/precanonical-2026-09-04/$accessEpochMigrationFile (migration under test) : $accessEpochHash"
