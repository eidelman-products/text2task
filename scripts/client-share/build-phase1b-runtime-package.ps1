...
.DESCRIPTION
  This script:
    ...
    - reads the seven pre-canonical source migrations from the canonical
...
# ---------------------------------------------------------------------
# 4. Read and hash the seven authoritative historical source migrations.
#    Read-only against the pre-canonical migration archive -- only
#    Get-Content, never a write.
# ---------------------------------------------------------------------
...
foreach ($name in $sourceFiles) {
  ...
  $sourceRelativePaths[$name] = "docs/database/migration-archive/precanonical-2026-09-04/$name"
...
-- docs/database/migration-archive/precanonical-2026-09-04/ and re-run
...
$manifestLines.Add('')
$manifestLines.Add('- The generator is read-only against `docs/database/migration-archive/precanonical-2026-09-04/**` -- it only ever')
$manifestLines.Add('  calls `Get-Content -LiteralPath ... -Raw` there, and never writes to that')
$manifestLines.Add('  archive. Its only two write targets are this file and')
