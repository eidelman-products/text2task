<#
.SYNOPSIS
  Mechanically builds BOTH generated outputs of the Phase 3 rate-limit
  foundation runtime verification package:
    docs/client-share-phase3-rate-limit-runtime/02_APPLY_CLIENT_SHARE_THROUGH_PHASE3_RATE_LIMIT.sql
    docs/client-share-phase3-rate-limit-runtime/MANIFEST.md

.DESCRIPTION
  This is the Phase 3 rate-limit-foundation counterpart of
  scripts/client-share/build-phase2b-mapping-read-runtime-package.ps1,
  extended from nine source migrations to ten (adding
  202608130001_client_share_rate_limit_increment.sql), producing the same
  two generated outputs in a different package directory. It does not
  read, write or modify anything under docs/client-share-phase1b-runtime/,
  docs/client-share-phase1c-runtime/, docs/client-share-phase2b-mapping-
  read-runtime/ or any of their generator scripts.

  Same guarantees as the Phase 1B/1C/2B generators:
...
$packageDir = Join-Path $repoRoot 'docs\client-share-phase3-rate-limit-runtime'
...
$bundleFileName = '02_APPLY_CLIENT_SHARE_THROUGH_PHASE3_RATE_LIMIT.sql'
...
-- rate-limit atomic-increment foundation
...
begin
  if to_regclass('public.text2task_client_share_phase3_rate_limit_runtime_sentinel') is null then
    raise exception using errcode = 'P0001', message =
      'REFUSING TO RUN. The Phase 3 rate-limit runtime test sentinel was not found in this project. Run 01_CREATE_TEMP_TEST_FIXTURE.sql here first, and only ever inside a brand-new temporary Supabase project.';
  end if;
  ...
  if v_sentinel_kind is distinct from 'DISPOSABLE_PHASE_3_RATE_LIMIT_RUNTIME_TEST_PROJECT' then
    raise exception using errcode = 'P0001', message =
      'REFUSING TO RUN. The sentinel row in this project does not identify it as a disposable Phase 3 rate-limit runtime test project.';
  end if;
...
$otherPackageFiles = [ordered]@{
  ...
  '03_RUN_PHASE3_RATE_LIMIT_RUNTIME_TESTS.sql' = @{ Path = (Join-Path $packageDir '03_RUN_PHASE3_RATE_LIMIT_RUNTIME_TESTS.sql'); Origin = 'hand-authored' }
  ...
  '06_concurrency_runner.mjs' = @{ Path = (Join-Path $packageDir '06_concurrency_runner.mjs'); Origin = 'hand-authored' }
  'build-phase3-rate-limit-runtime-package.ps1' = @{ Path = $MyInvocation.MyCommand.Path; Origin = 'hand-authored (this generator)' }
}
...
$manifestLines.Add('# Client Share Link -- Phase 3 Rate-Limit Foundation Runtime Verification Package Manifest')
$manifestLines.Add('')
$manifestLines.Add('Mechanically generated by `scripts/client-share/build-phase3-rate-limit-runtime-package.ps1`.')
...
$manifestLines.Add("| 3 | ``02_APPLY_CLIENT_SHARE_THROUGH_PHASE3_RATE_LIMIT.sql`` | **generated** (mechanically assembled from the ten source migrations below) | ``$bundleHash`` |")
$manifestLines.Add("| 4 | ``03_RUN_PHASE3_RATE_LIMIT_RUNTIME_TESTS.sql`` | hand-authored | ``$($otherPackageHashes['03_RUN_PHASE3_RATE_LIMIT_RUNTIME_TESTS.sql'])`` |")
...
$manifestLines.Add("| 7 | ``06_concurrency_runner.mjs`` | hand-authored | ``$($otherPackageHashes['06_concurrency_runner.mjs'])`` |")
$manifestLines.Add('| 8 | `MANIFEST.md` | **generated** (this file) | *(intentionally not embedded -- see below)* |')
$manifestLines.Add("| 9 | ``build-phase3-rate-limit-runtime-package.ps1`` | hand-authored (this generator) | ``$($otherPackageHashes['build-phase3-rate-limit-runtime-package.ps1'])`` |")
$manifestLines.Add('')
...
$manifestLines.Add('')
$manifestLines.Add('Hashes 1-9 should match the values already recorded in')
$manifestLines.Add('`docs/client-share-phase2b-mapping-read-runtime/MANIFEST.md` for the same nine')
$manifestLines.Add('files -- those migrations have not changed since that package was generated')
$manifestLines.Add('and runtime-verified. Hash 10 is new in this package.')
$manifestLines.Add('')
$manifestLines.Add('## To reproduce or re-verify these hashes')
$manifestLines.Add('')
$manifestLines.Add('```')
$manifestLines.Add('powershell -File scripts/client-share/build-phase3-rate-limit-runtime-package.ps1')
$manifestLines.Add('```')
$manifestLines.Add('')
...
$manifestLines.Add('  value appears anywhere in this package, including the concurrency runner --')
$manifestLines.Add('  it reads both required values from environment variables only.')
