<#
.SYNOPSIS
  Mechanically builds BOTH generated outputs of the Phase 1C runtime
  verification package:
    docs/client-share-phase1c-runtime/02_APPLY_CLIENT_SHARE_THROUGH_PHASE1C.sql
    docs/client-share-phase1c-runtime/MANIFEST.md

.DESCRIPTION
  This is the Phase 1C counterpart of
  scripts/client-share/build-phase1b-runtime-package.ps1, extended from
  seven source migrations to eight (adding
  202608110001_client_share_publication_intent.sql), producing the same
  ...
  or the Phase 1B generator script.

  Same guarantees as the Phase 1B generator:
    - never modifies the eight source migration files
    - never runs any SQL
...
$packageDir = Join-Path $repoRoot 'docs\client-share-phase1c-runtime'
$reportPath = Join-Path $repoRoot 'docs\TEXT2TASK_CLIENT_SHARE_LINK_PHASE_1C_IMPLEMENTATION_REPORT.md'
...
$bundleFileName = '02_APPLY_CLIENT_SHARE_THROUGH_PHASE1C.sql'
...
# ---------------------------------------------------------------------
# 4. Read and hash the eight authoritative source migrations. Read-only
...
Write-Host 'Order verification passed: all eight source migrations, each exactly once, each with a matching source-hash comment.'
...
$otherPackageFiles = [ordered]@{
  'TEXT2TASK_CLIENT_SHARE_LINK_PHASE_1C_IMPLEMENTATION_REPORT.md' = @{ Path = $reportPath; Origin = 'hand-authored' }
  ...
  '03_RUN_PHASE1C_RUNTIME_TESTS.sql' = @{ Path = (Join-Path $packageDir '03_RUN_PHASE1C_RUNTIME_TESTS.sql'); Origin = 'hand-authored' }
  ...
  'build-phase1c-runtime-package.ps1' = @{ Path = $MyInvocation.MyCommand.Path; Origin = 'hand-authored (this generator)' }
}
...
$manifestLines.Add('# Client Share Link -- Phase 1C Runtime Verification Package Manifest')
$manifestLines.Add('')
$manifestLines.Add('Mechanically generated by `scripts/client-share/build-phase1c-runtime-package.ps1`.')
...
$manifestLines.Add('|---|---|---|---|')
$manifestLines.Add("| 1 | ``TEXT2TASK_CLIENT_SHARE_LINK_PHASE_1C_IMPLEMENTATION_REPORT.md`` | hand-authored | ``$($otherPackageHashes['TEXT2TASK_CLIENT_SHARE_LINK_PHASE_1C_IMPLEMENTATION_REPORT.md'])`` |")
...
$manifestLines.Add("| 4 | ``02_APPLY_CLIENT_SHARE_THROUGH_PHASE1C.sql`` | **generated** (mechanically assembled from the eight source migrations below) | ``$bundleHash`` |")
$manifestLines.Add("| 5 | ``03_RUN_PHASE1C_RUNTIME_TESTS.sql`` | hand-authored | ``$($otherPackageHashes['03_RUN_PHASE1C_RUNTIME_TESTS.sql'])`` |")
...
$manifestLines.Add("| 9 | ``build-phase1c-runtime-package.ps1`` | hand-authored (this generator) | ``$($otherPackageHashes['build-phase1c-runtime-package.ps1'])`` |")
$manifestLines.Add('')
$manifestLines.Add('### Why row 8 has no embedded hash')
$manifestLines.Add('')
$manifestLines.Add('Same reasoning as the Phase 1B manifest: embedding would require a')
...
$manifestLines.Add('')
$manifestLines.Add('Hashes 1-7 should match the values already recorded in')
$manifestLines.Add('`docs/client-share-phase1b-runtime/MANIFEST.md` for the same seven files --')
$manifestLines.Add('those migrations have not changed since Phase 1B''s own package was')
$manifestLines.Add('generated and runtime-verified. Hash 8 is new in this package.')
$manifestLines.Add('')
$manifestLines.Add('## To reproduce or re-verify these hashes')
$manifestLines.Add('')
$manifestLines.Add('```')
$manifestLines.Add('powershell -File scripts/client-share/build-phase1c-runtime-package.ps1')
$manifestLines.Add('```')
$manifestLines.Add('')
...
$manifestLines.Add('  eight source migrations above, mechanically verified every run.')
$manifestLines.Add('- This package does not modify the separate, existing')
$manifestLines.Add('  `docs/client-share-phase1b-runtime/` package or its own generator in any')
$manifestLines.Add('  way.')
