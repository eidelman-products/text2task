<#
.SYNOPSIS
  Mechanically builds BOTH generated outputs of the Phase 3 application
  implementation runtime verification package:
    docs/client-share-phase3-runtime/02_APPLY_CLIENT_SHARE_THROUGH_PHASE3_APPLICATION.sql
    docs/client-share-phase3-runtime/MANIFEST.md

.DESCRIPTION
  This is the Phase 3 application-implementation counterpart of
  scripts/client-share/build-phase3-rate-limit-runtime-package.ps1. Phase 3
  application code (session exchange, cookies, grants, the public route,
  the PIN flow, the projection read) added NO new migration, so this
  generator bundles the SAME ten already-authorized source migrations,
  producing the same two generated outputs in a different package
  directory. It does not read, write or modify anything under
  docs/client-share-phase1b-runtime/, docs/client-share-phase1c-runtime/,
  docs/client-share-phase2b-mapping-read-runtime/,
  docs/client-share-phase3-rate-limit-runtime/, or any of their generator
  scripts.

  Same guarantees as the Phase 1B/1C/2B/3-rate-limit generators:
    - never modifies the ten source migration files
    - never runs any SQL
...
$packageDir = Join-Path $repoRoot 'docs\client-share-phase3-runtime'
...
$bundleFileName = '02_APPLY_CLIENT_SHARE_THROUGH_PHASE3_APPLICATION.sql'
...
# ---------------------------------------------------------------------
# 4. Read and hash the ten authoritative source migrations. Read-only
...
-- Package
-- File 02: Exact Client Share migration bundle through the Phase 3
-- rate-limit atomic-increment foundation (unchanged -- Phase 3
-- application code added no new migration)
-- (MECHANICALLY GENERATED)
--
-- Generated by
-- scripts/client-share/build-phase3-application-runtime-package.ps1.
-- Do not hand-edit this file -- edit the ten source migrations under
...
begin
  if to_regclass('public.text2task_client_share_phase3_application_runtime_sentinel') is null then
    raise exception using errcode = 'P0001', message =
      'REFUSING TO RUN. The Phase 3 application runtime test sentinel was not found in this project. Run 01_CREATE_TEMP_TEST_FIXTURE.sql here first, and only ever inside a brand-new temporary Supabase project.';
  end if;
  ...
  if v_sentinel_kind is distinct from 'DISPOSABLE_PHASE_3_APPLICATION_RUNTIME_TEST_PROJECT' then
    raise exception using errcode = 'P0001', message =
      'REFUSING TO RUN. The sentinel row in this project does not identify it as a disposable Phase 3 application runtime test project.';
  end if;
...
-- check for constraints, indexes, defaults and exact-signature RPC
...
from (
  values
    ...
    ('function', 'increment_share_rate_limit_bucket(text,text,text,smallint,uuid,integer)'),
...
Write-Host 'Order verification passed: all ten source migrations, each exactly once, each with a matching source-hash comment.'
...
$otherPackageFiles = [ordered]@{
  ...
  '03_RUN_PHASE3_APPLICATION_RUNTIME_TESTS.sql' = @{ Path = (Join-Path $packageDir '03_RUN_PHASE3_APPLICATION_RUNTIME_TESTS.sql'); Origin = 'hand-authored' }
  ...
  'PHASE3_BROWSER_WEBVIEW_CHECKLIST.md' = @{ Path = (Join-Path $packageDir 'PHASE3_BROWSER_WEBVIEW_CHECKLIST.md'); Origin = 'hand-authored' }
  'build-phase3-application-runtime-package.ps1' = @{ Path = $MyInvocation.MyCommand.Path; Origin = 'hand-authored (this generator)' }
}
...
$manifestLines.Add('# Client Share Link -- Phase 3 Application Runtime Verification Package Manifest')
$manifestLines.Add('')
$manifestLines.Add('Mechanically generated by `scripts/client-share/build-phase3-application-runtime-package.ps1`.')
...
$manifestLines.Add('')
$manifestLines.Add('## Package files (all nine approved files)')
$manifestLines.Add('')
...
$manifestLines.Add("| 3 | ``02_APPLY_CLIENT_SHARE_THROUGH_PHASE3_APPLICATION.sql`` | **generated** (mechanically assembled from the ten source migrations below) | ``$bundleHash`` |")
$manifestLines.Add("| 4 | ``03_RUN_PHASE3_APPLICATION_RUNTIME_TESTS.sql`` | hand-authored | ``$($otherPackageHashes['03_RUN_PHASE3_APPLICATION_RUNTIME_TESTS.sql'])`` |")
...
$manifestLines.Add("| 7 | ``PHASE3_BROWSER_WEBVIEW_CHECKLIST.md`` | hand-authored | ``$($otherPackageHashes['PHASE3_BROWSER_WEBVIEW_CHECKLIST.md'])`` |")
$manifestLines.Add('| 8 | `MANIFEST.md` | **generated** (this file) | *(intentionally not embedded -- see below)* |')
$manifestLines.Add("| 9 | ``build-phase3-application-runtime-package.ps1`` | hand-authored (this generator) | ``$($otherPackageHashes['build-phase3-application-runtime-package.ps1'])`` |")
$manifestLines.Add('')
$manifestLines.Add('### Why row 8 has no embedded hash')
$manifestLines.Add('')
$manifestLines.Add('Same reasoning as every prior Client Share runtime manifest: embedding')
...
$manifestLines.Add('')
$manifestLines.Add('Applied by file 02, in this exact order, verbatim and unmodified. Identical')
$manifestLines.Add('to the ten migrations already bundled by the Phase 3 rate-limit-foundation')
$manifestLines.Add('package -- Phase 3 application implementation added no new migration.')
$manifestLines.Add('')
...
$manifestLines.Add('')
$manifestLines.Add('These ten hashes should match the values already recorded in')
$manifestLines.Add('`docs/client-share-phase3-rate-limit-runtime/MANIFEST.md` for the same ten')
$manifestLines.Add('files -- those migrations have not changed since that package was generated')
$manifestLines.Add('and runtime-verified.')
$manifestLines.Add('')
$manifestLines.Add('## To reproduce or re-verify these hashes')
$manifestLines.Add('')
$manifestLines.Add('```')
$manifestLines.Add('powershell -File scripts/client-share/build-phase3-application-runtime-package.ps1')
$manifestLines.Add('```')
$manifestLines.Add('')
...
$manifestLines.Add('  ten source migrations above, mechanically verified every run.')
$manifestLines.Add('- This package does not modify any separate, existing Client Share runtime')
$manifestLines.Add('  package or generator in any way.')
