...
function buildRequest() {
  return new NextRequest(`http://localhost/api/share-links/${VALID_UUID}/activate`, {
    method: "POST",
  });
}
...
function validActivateData() {
  return {
    linkId: VALID_UUID,
    publicId: "abcdefgh12345678",
    state: "active",
    configurationVersion: 1,
    activatedAt: "2026-08-06T00:00:00Z",
    secret: VALID_SECRET,
  };
}
...
describe("POST /api/share-links/[id]/activate - feature gate", () => {
...
describe("POST /api/share-links/[id]/activate - validation", () => {
  ...
  it("canonicalizes an uppercase id to lowercase before calling the repository", async () => {
    ...
    activateShareLinkMock.mockResolvedValue({ ok: true, data: validActivateData() });

    await POST(buildRequest(), buildContext(VALID_UUID.toUpperCase()));

    expect(activateShareLinkMock).toHaveBeenCalledWith(expect.anything(), VALID_UUID);
  });
});

describe("POST /api/share-links/[id]/activate - authentication", () => {
...
describe("POST /api/share-links/[id]/activate - repository outcomes", () => {
  beforeEach(() => {
    getUserMock.mockResolvedValue({ data: { user: { id: "user-1" } }, error: null });
  });
  ...
  it("logs only a fixed category, never error.name or error.message, even with sensitive markers", async () => {
    ...
    activateShareLinkMock.mockRejectedValue(error);
    ...
    expect(loggedPayload).toEqual({
      stage: "share_links.activate",
      category: "Error",
    });
  });

  it("returns {ok:true,data} on success, including the secret, calling exactly one repository function", async () => {
    const data = validActivateData();
    activateShareLinkMock.mockResolvedValue({ ok: true, data });
  ...
  it("never leaks ciphertext, digest, nonce, authTag or a raw database error code", async () => {
    const data = validActivateData();
    activateShareLinkMock.mockResolvedValue({ ok: true, data });
...
describe("POST /api/share-links/[id]/activate - explicit no-store headers on every response branch", () => {
