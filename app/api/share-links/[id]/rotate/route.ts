...
import { rotateShareLinkSecret } from "@/lib/share/share-links-repository.server";
 ...
 * failure. This route's own success response carries a freshly generated
 * raw share secret and must never be cached (see
...
export async function POST(_req: NextRequest, context: RouteContext) {
  try {
    ...
    const result = await rotateShareLinkSecret(supabase, parsedId.data.id);
      ...
      switch (result.error.code) {
        ...
        case "SHARE_LINK_STATE_CONFLICT":
          return errorResponse(
            ...
            "Share link secret cannot be rotated in its current state.",
            409
          );
        case "SHARE_LINK_SECRET_UNAVAILABLE":
          return errorResponse(
            "SHARE_LINK_SECRET_UNAVAILABLE",
            "Failed to rotate the share link secret.",
            500
          );
        default:
          return errorResponse(
    ...
    logShareLinksRouteError("share_links.rotate", error);
