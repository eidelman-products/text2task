...
import { disableShareLink } from "@/lib/share/share-links-repository.server";
...
export async function POST(_req: NextRequest, context: RouteContext) {
  try {
    ...
    const result = await disableShareLink(supabase, parsedId.data.id);
      ...
      switch (result.error.code) {
        ...
        case "SHARE_LINK_STATE_CONFLICT":
          return errorResponse(
            ...
            "Share link is not active.",
            409
          );
        default:
          return errorResponse(
            "INTERNAL_ERROR",
            "Failed to disable the share link.",
            500
          );
      }
    }
    ...
    logShareLinksRouteError("share_links.disable", error);
