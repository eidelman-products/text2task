...
import { revealShareLinkSecret } from "@/lib/share/share-links-repository.server";
 ...
 * nonce, auth tag or any other secret material.
 */
 ...
 * failure. This route's own success response carries the currently
 * stored raw share secret and must never be cached (see
 * app/api/share-links/[id]/activate/route.ts for the full rationale).
 */
...
export async function POST(_req: NextRequest, context: RouteContext) {
  try {
    ...
    const result = await revealShareLinkSecret(supabase, parsedId.data.id);
      ...
      switch (result.error.code) {
        ...
        case "SHARE_LINK_STATE_CONFLICT":
          return errorResponse(
            ...
            "Share link secret cannot be revealed in its current state.",
            409
          );
        case "SHARE_LINK_SECRET_UNAVAILABLE":
          return errorResponse(
            "SHARE_LINK_SECRET_UNAVAILABLE",
            "Failed to reveal the share link secret.",
            500
          );
        default:
          return errorResponse(
    ...
    logShareLinksRouteError("share_links.reveal", error);
